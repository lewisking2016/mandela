...
import { Card, CardHead, KpiCard, Meter, Delta, Money, StatusPill, EmptyState, SerifHeader } from "@mandela/ui";
import { AppShell } from "./AppShell";
import Link from "next/link";

/**
 * /app — the role's world, in comp 02's bento. The shell (tabs, labels,
 * prime question) is resolved from the session role + bootstrap payload;
 * the home screen is one query per role, every number from the school DB.
 */
...
function FeedRows({ rows }: { rows: { title: string; meta: string; pill: { tone: "ok" | "warn" | "danger" | "neutral"; label: string } }[] }) {
  return (
    <div>
      {rows.map((r, i) => (
        <div key={i} className="flex items-center justify-between gap-s3 border-t border-paper-200 py-s3 first:border-t-0 first:pt-0">
    ...
    <div className="grid gap-s3h">
      ...
      <div className="grid gap-s3h sm:grid-cols-2 xl:grid-cols-3">
            ...
            {data.last7.map((d, i) => {
              ...
              return (
                <div key={i} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-1.5">
                  <div
                    aria-hidden
                    className={`w-full rounded-md rounded-b-sm ${empty ? "bg-deep-line" : "bg-white"}`}
        ...
        {canSeeMoney ? (
          ...
          <KpiCard label="Active learners" value={data.count} note="learners on roll" />
        )}

        <KpiCard
          label="Attendance today"
          value={<>{attRate}<span className="text-xl font-medium text-ink-500">%</span></>}
          ...
          {payments.length === 0 ? (
            ...
            <div>
              {payments.slice(0, 4).map((p) => (
                <div key={p.receipt_no} className="flex items-center justify-between gap-s3 border-t border-paper-200 py-s3 first:border-t-0 first:pt-0">
          ...
          <div className="grid gap-2.5">
            {(role === "teacher"
              ...
              <Link
                key={a.href}
                href={a.href}
                className="flex min-h-[48px] items-center justify-between rounded-sm border border-paper-200 px-4 py-3 text-[13.5px] font-semibold text-text hover:bg-paper-50"
              >
                {a.label}
      ...
      {/* Row 3 — collections + announcements */}
      {canSeeMoney ? (
        <Card>
          <CardHead
          ...
          {collections.length === 0 ? (
            ...
            <div>
              {collections.slice(0, 4).map((c) => {
                ...
                return (
                  <div key={c.class} className="flex items-center justify-between gap-s3 border-t border-paper-200 py-s3 first:border-t-0 first:pt-0">
                    <div className="min-w-0">
